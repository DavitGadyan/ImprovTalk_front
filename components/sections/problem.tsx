...
export function Problem({ persona, index }: { persona: Persona; index: number }) {
  ...
  return (
    <Section id="problem" index={index} label={label} title={title} intro={intro}>
      <motion.ol
        variants={stagger}
        initial="hidden"
        whileInView="visible"
        viewport={viewportOnce}
        className="grid gap-px overflow-hidden rounded-card border border-line bg-line md:grid-cols-3"
      >
        {points.map((p, i) => (
          <motion.li
            key={p.title}
            variants={popIn}
            className="bg-canvas p-7 transition-colors duration-300 hover:bg-surface md:p-8"
          >
            <span className="numeric text-micro font-semibold tracking-[0.16em] text-muted">
              0{i + 1}
            </span>
